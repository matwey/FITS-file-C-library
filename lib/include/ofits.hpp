...
public:
    ...
    public:
            ...
            : parent_ofits_(parent_ofits), headers_written_(0), header_block_(2880, ' '), offset_(offset)
        {
            ...
            const std::size_t data_block_size = (((naxis_product * std::abs(bitpix) / 8) + kSizeHeaderBlock - 1) / kSizeHeaderBlock) * kSizeHeaderBlock;
            const std::size_t new_offset = get_header_block_count() + data_block_size;

            parent_ofits_.update_offset(round_offset(80 * headers_written_) + new_offset);
        }
    ...
    private:
        ...
        {
            return (headers_written_ + kSizeHeaderBlock - 1) / kSizeHeaderBlock;
        }
